...
echo "Starting miner for ${DURATION} seconds..."
# Cross-platform timeout implementation (timeout command doesn't exist on macOS)
"$CLIENT_BIN" -server "localhost:$SERVER_PORT" >out.log 2>&1 &
MINER_PID=$!
sleep "$DURATION"
...
ZERO_HASH_COUNT=$(grep -c "Hash rate: 0 H/s" out.log 2>/dev/null || echo "0")
ZERO_HASH_COUNT=$(echo "$ZERO_HASH_COUNT" | tr -d '\n')
TOTAL_HASH_COUNT=$(grep -c "Hash rate:" out.log 2>/dev/null || echo "0")
TOTAL_HASH_COUNT=$(echo "$TOTAL_HASH_COUNT" | tr -d '\n')
...
echo "Querying API for blockchain stats..."
