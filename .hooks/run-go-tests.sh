...
run_tests() {
	...
	if [[ "${TESTS_TO_RUN}" == 'coverage' ]]; then
		...
		IFS=$'\n' read -r -d '' -a modified_files < <(git diff --name-only --cached | grep '\.go$' && printf '\0')

		if [[ ${#modified_files[@]} -eq 0 ]]; then
			echo "No modified Go files found" | tee -a "$LOGFILE"
			return 0
		fi
		...
		if [[ ${#pkg_dirs[@]} -gt 0 ]]; then
			IFS=$'\n' read -r -d '' -a pkg_dirs < <(printf '%s\n' "${pkg_dirs[@]}" | sort -u && printf '\0')
			unset IFS
