...
- [Overview](#overview)
- [Features](#features)
- [Why Should I Run This Simulation in Red Team Operations?](#Why-Should-I-Run-This-Simulation-in-Red-Team-Operations?)
...
## Why Should I Run This Simulation in Red Team Operations?

Organizations typically use several methods to detect and prevent cryptocurrency mining on their networks, with traffic monitoring and blocking being a primary defense. This involves monitoring for connections to known mining pools and blocking those domains and IP addresses at the firewall, as mining generates distinctive network patterns with persistent outbound connections to pool servers. Organizations implement this through threat intelligence feeds that maintain updated lists of known mining pool domains and IP addresses from services like abuse.ch, AlienVault OTX, or commercial providers. DNS-based blocking using services like Cisco Umbrella or Cloudflare Gateway includes mining pool domains in their threat categories, blocking or redirecting DNS requests when devices attempt to resolve mining pool domains. This DNS-level blocking is typically the first step organizations deploy to stop crypto-mining on corporate enterprises. However, once DNS solutions are in place, organizations must address the next layer of defense: CPU/GPU mining detection. This is where Red Team operations become valuable. By running simulations using tools like RedTeamCoin (https://github.com/xyplex3/RedTeamCoin), red teams can test whether an organization's endpoint detection and monitoring capabilities can identify cryptocurrency mining activity that bypasses DNS controls, validating that the organization has adequate visibility into resource-intensive mining processes running on corporate endpoints even when network-level blocking is already implemented.
