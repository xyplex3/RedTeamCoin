...
// Package main implements CUDA-based GPU mining for NVIDIA GPUs.
//
// This file is compiled only when both the cuda and cgo build tags are enabled.
// It provides hardware-accelerated proof-of-work mining using NVIDIA CUDA,
// with automatic fallback to CPU mining if GPU operations fail.
//
// # Build Requirements
//
// To build with CUDA support:
//   - Install NVIDIA CUDA Toolkit from https://developer.nvidia.com/cuda-downloads
//   - Compile the CUDA kernel: nvcc -c -m64 -O3 client/mine.cu -o client/mine_cuda.o
//   - Build: CGO_ENABLED=1 go build -tags cuda -o bin/client ./client
//
// # Detection
//
// GPU detection uses nvidia-smi to query available NVIDIA hardware.
// Performance can be 10-100x faster than CPU mining for large nonce ranges.
package main
